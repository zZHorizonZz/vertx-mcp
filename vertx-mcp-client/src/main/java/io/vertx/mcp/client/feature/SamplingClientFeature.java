...
/**
 * The SamplingClientFeature class implements the ClientFeatureBase and provides functionality to handle sampling-related operations. This feature allows the client to respond to
 * sampling/createMessage requests from the server, enabling the server to sendRequest LLM completions from the client.
 *
 * @version 2025-06-18
...
public class SamplingClientFeature extends ClientFeatureBase {
  ...
  private Future<JsonObject> handleCreateMessage(JsonRequest request) {
    ...
    return samplingHandler.apply(createMessageRequest).map(result -> JsonResponse.success(request, result.toJson()).toJson());
  }
}
